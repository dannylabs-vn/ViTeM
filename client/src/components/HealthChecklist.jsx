...
const tagConfig = {
  GREEN: { bg: "bg-emerald-50", border: "border-emerald-200", text: "text-emerald-700", dot: "bg-emerald-500", label: "Normal", icon: CheckCircle2 },
  YELLOW: { bg: "bg-amber-50", border: "border-amber-200", text: "text-amber-700", dot: "bg-amber-400", label: "Needs Monitoring", icon: AlertCircle },
  RED: { bg: "bg-red-50", border: "border-red-200", text: "text-red-700", dot: "bg-red-500", label: "Danger", icon: AlertTriangle },
};
    ...
    >
      ...
      <div className={`px-5 py-4 flex items-center justify-between ${urgencyConfig.bg} border-b ${urgencyConfig.border}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-gray-900 text-sm">Health Examination Form</h4>
        ...
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold ${urgencyConfig.bg} ${urgencyConfig.text} border ${urgencyConfig.border}`}>
          ...
          {overallUrgency === 'RED' ? 'Red Case' : overallUrgency === 'YELLOW' ? 'Yellow Case' : 'Normal'}
        </div>
      </div>
      ...
      <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-4 text-xs font-medium">
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-gray-600">{greenCount} normal</span>
          </span>
          {yellowCount > 0 && (
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-amber-400"></span>
              <span className="text-gray-600">{yellowCount} needs monitoring</span>
            </span>
          )}
          {redCount > 0 && (
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-red-500"></span>
              <span className="text-gray-600">{redCount} danger</span>
            </span>
          )}
        </div>
